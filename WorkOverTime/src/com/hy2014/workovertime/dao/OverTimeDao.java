...
public class OverTimeDao implements  BaseDao
{
	...
	/**
	 * 计算工资
	 * @param fromDate 开始日期    8位整数
	 * @param toDate 结束日期   8位整数
	 * @param isOvertimeSalary true计算加班工资,false计算总工资
	 * @return
	 */
	public float calOvertimeSalary(int fromDate, int toDate,boolean isOvertimeSalary)
	{
		//String sql = "select hours from overTimeTb where date>=  ? and date<=? and dateType=";
	  ...
	  // LogUtil.e("基本工资---->"+baseSlary);
	   ...
	   //big bang  boom
		try
			...
			return isOvertimeSalary?
					(normalHours*normalSalry + weekHours*weekSalry + holidayHours*holidaySalry):
	...
	{
		String sql="insert into overTimeTb(" +
				"date," +
				"dateType," +
				"hours," +
				"fromTime," +
